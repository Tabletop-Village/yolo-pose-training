...
# Paths adapted from the original Windows-path version (card-sorter/card-scanner,
# `cropping` branch) to this machine's layout. `images/` here is a flat dir of
# symlinks into the already-downloaded tcgcsv catalog images (siglip-scanner's
# vectors/images/3/*/*.jpg), reusing that download instead of re-fetching.
BACKGROUNDS_DIR = os.path.expanduser("~/yolo-pose-training/backgrounds/train2017")
OUTPUT_DIR = os.path.expanduser("~/yolo-pose-training/dataset/train")
...
def overlay2(background, overlay_img, text, num):
    ...
    #calculate the 4 corners in a FIXED order: source (0,0)/(w,0)/(w,h)/(0,h)
    # (i.e. TL/TR/BR/BL of the *unflipped* overlay geometry) transformed by M.
    # This order is what makes the labels usable as pose keypoints: it's
    # consistent across every example regardless of the random 180 flip
    # above (that flip only changes which printed *content* ends up at each
    # geometric corner, not the fixed index order corners are emitted in).
    # Downstream, preprocess.py's order_points() already re-derives
    # geometric TL/TR/BR/BL purely from pixel coordinates, so we don't need
    # "true" semantic corner identity -- just a stable per-example ordering.
    ...
    point_vals = np.concatenate((pts[0], pts[1]))  # [TL, TR, BR, BL], pixel coords

    # A rotated card can extend past the frame even though its unrotated
    # placement was chosen to fit -- clamp each corner to the image bounds
    # for the label (ultralytics' loader rejects the WHOLE image's labels,
    # not just the offending point, if any coordinate falls outside a ~1%
    # tolerance of [0, 1]). Clamped corners are marked visibility=1
    # (labeled but not exactly located, since the true corner was
    # off-frame) instead of 2 (labeled+visible) for genuinely in-frame
    # corners; this still keeps the earlier "not clamped to the image"
    # leniency in spirit -- partially-offscreen cards still produce a
    # usable box/keypoints, just clipped to what the loader will accept.
    visible = (point_vals[:, 0] >= 0) & (point_vals[:, 0] <= w_background) & \
              (point_vals[:, 1] >= 0) & (point_vals[:, 1] <= h_background)
    point_vals[:, 0] = np.clip(point_vals[:, 0], 0, w_background)
    point_vals[:, 1] = np.clip(point_vals[:, 1], 0, h_background)
    xs = point_vals[:, 0]
    ys = point_vals[:, 1]

    # YOLO pose label: class cx cy w h x1 y1 v1 x2 y2 v2 x3 y3 v3 x4 y4 v4
    # (all coords normalized 0-1 by image dims).
    x_min, x_max = xs.min(), xs.max()
    y_min, y_max = ys.min(), ys.max()
    cx = (x_min + x_max) / 2 / w_background
    cy = (y_min + y_max) / 2 / h_background
    bw = (x_max - x_min) / w_background
    bh = (y_max - y_min) / h_background

    out = "0 %.3f %.3f %.3f %.3f " % (cx, cy, bw, bh)
    for (x, y), vis in zip(point_vals, visible):
        out += "%.3f %.3f %d " % (x / w_background, y / h_background, 2 if vis else 1)
    out += "\n"
    text.write(out)
    return result
...
if __name__ == "__main__":
    ...
    for c, i in enumerate(images):
        todo.append((
            os.path.join(BACKGROUNDS_DIR, i),
            str(c))
        )
        #if c > 50:
        #    break
